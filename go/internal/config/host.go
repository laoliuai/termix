...
type HostConfig struct {
	...
	// `enable_proxy` lived here in v0.3.0–v0.4.2 to gate a process-wide
	// `os.Unsetenv` of HTTPS_PROXY etc. The relay WSS now bypasses the
	// proxy via its own dedicated http.Client, so the policy is moot —
	// JSON unmarshal silently ignores the legacy key in old host.json
